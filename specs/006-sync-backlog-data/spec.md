...
- **跨時區切換**: 當使用者切換系統時區時，已儲存的日期字串 (`YYYY-MM-DD`) 應視為本地日期而非 UTC，以維持顯示的一致性。
...
- **FR-006**: **本地時區一致性 (Local Timezone Consistency)**: 系統必須使用使用者的本地時區 (Local Timezone) 來處理所有日期邏輯。取得「今日」日期時應考慮時區偏移，確保排程日期與使用者的生理時間同步，避免使用 ISO UTC 時間導致日期錯誤。
