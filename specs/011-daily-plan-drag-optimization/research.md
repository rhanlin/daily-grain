...
- **Affected Files**: 
    ...
    - (Any other components using `useLongPress`)
...
- **Conflict 1**: "Sort by Matrix" (automated) vs Dragging (manual). **Decision**: Disable drag handles when matrix sorting is active (FR-006).
- **Conflict 2**: `DailyPlanPage` reorder handler requires a `'daily-plan'` container ID which was missing in `DailyPlanView`. **Decision**: Explicitly set the ID in `SortableContext` (FR-007).
