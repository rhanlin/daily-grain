...
- [x] T003 Update `repository.subtasks.syncParentTaskStatus` in `src/lib/repository.ts` to implement the redefined Status Engine logic (FR-001 to FR-005): (Completed)
...
**Checkpoint**: Core logic ready - Task status now reflects cumulative subtask progress.

---

## Phase 3: User Story 1 - Automatic Completion Logic (Priority: P1) 🎯 MVP

**Goal**: Seamless automatic status transitions based on subtask updates.
...
- [x] T008 [US1] Ensure `repository.subtasks.create` and `repository.subtasks.delete` trigger `syncParentTaskStatus` to handle dynamic reversion (FR-007). (Completed)
...
**Checkpoint**: User Story 1 functional - Automation is active and accurate.

---

## Phase 4: User Story 2 - Manual Completion Override (Priority: P2)

**Goal**: Allow users to terminate tasks with recurring items.
...
- [x] T010 [US2] Update Task management UI (if needed) to allow manual DONE toggle for tasks with daily items. (Completed)
- [x] T011 [US2] Add a confirmation dialog or warning in the UI when manually completing a Task with unfinished recurring subtasks (FR-006). (Completed)

**Checkpoint**: User Story 2 functional - Manual control is preserved.
...
- [x] T012 Verify SC-003 (Performance < 100ms) by benchmarking complex tasks with 10+ subtasks. (Completed)
...
1. **Phase 2 (Foundational)** is the primary blocker. Logic MUST be in the repository before UI behavior can be verified.
